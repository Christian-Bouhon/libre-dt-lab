...
void reload_defaults(dt_iop_module_t *self)
{
  ...
  if(scene_raw && self->multi_priority == 0)
  {
    ...
    // For basecurve and 3DCF workflows, exposure stays at 0.0 IL
    // (basecurve/3DCF handle the tone mapping)
    // For other scene-referred workflows (filmic, sigmoid, AgX), use 0.7 IL default
    const char *workflow = dt_conf_get_string_const("plugins/darkroom/workflow");
    const gboolean is_basecurve_or_3dcf = workflow && 
      (strcmp(workflow, "scene-referred (3DCF)") == 0 || 
       strcmp(workflow, "scene-referred (basecurve)") == 0);
    d->exposure = mono ? 0.0f : (is_basecurve_or_3dcf ? 0.0f : 0.7f);
